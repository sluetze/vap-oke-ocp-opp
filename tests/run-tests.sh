...
# Prerequisites: kubectl, cluster with VAP support, cluster-admin.
...
kubectl apply -f "$REPO_ROOT/policies/operator-valid-subscription.yaml"
kubectl apply -f "$REPO_ROOT/policies/operator-subscription-matches-cluster.yaml"
kubectl apply -f "$REPO_ROOT/bindings/operator-valid-subscription-binding.yaml"
kubectl apply -f "$REPO_ROOT/bindings/operator-subscription-matches-cluster-binding.yaml"

# Helpers: expect pass or fail for a fixture
apply_expect_pass() {
  local f="$1"
  local name=$(basename "$f")
  if kubectl apply -f "$f" --namespace "$NAMESPACE" --dry-run=server 2>&1; then
    echo "  PASS: $name"
    return 0
  else
